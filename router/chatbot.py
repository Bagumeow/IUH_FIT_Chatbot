...
async def chat_history(access_token: str = Depends(oauth2_scheme)):
    ...
    cursor.execute("SELECT session_id,content FROM history_chat WHERE user_id = %s AND model_type = %s", (user_id, "Gemini"))
...
async def chat_history_gpt(access_token: str = Depends(oauth2_scheme)):
    ...
    cursor = create_cursor(conn)
    cursor.execute("SELECT session_id,content FROM history_chat WHERE user_id = %s AND model_type = %s", (user_id, "GPT"))
    chat_history = cursor.fetchall()
    conn.close()
    all_history = {}
    if chat_history:
